 ...
 */
export const postObtainServiceId = async (req: Request, res: Response) => {
  Service.findOne({ user_email: req.body.user_email }, (err, dbService) => {
    if (err) {
 ...
 */
export const getServiceName = async (req: Request, res: Response) => {
  const serviceId = Base64.decode(req.params.service_id);
  Service.findOne({ service_id: serviceId }, (err, dbService) => {
    if (err) {
      return res.status(constants.HTTP_STATUS_INTERNAL_SERVER_ERROR).send({ error: 'Some error occured' });
    }
    if (!dbService) {
...
export const getAvailableServices = async (req: Request, res: Response) => {
  Service.find({ hidden: false }, 'service_id name image description', (err, dbServices) => {
    if (err) {
      return res.status(constants.HTTP_STATUS_INTERNAL_SERVER_ERROR).send({ error: 'Some error occured' });
    }
    if (!dbServices) {
 ...
 */
export const getServiceSettings = async (req: Request, res: Response) => {
  const serviceId = Base64.decode(req.params.service_id);
  Service.findOne({ service_id: serviceId }, (err, dbService) => {
    if (err) {
      return res.status(constants.HTTP_STATUS_INTERNAL_SERVER_ERROR).send({ error: 'Some error occured' });
 ...
 * Updates Service
 */
export const postUpdateService = async (req: any, res: Response) => {
      ...
      .then(async (isMatch) => {
        if (isMatch) {
          ...
          if (!req.file && !deleteImage) {
            ...
            Service.findOne({ user_email: service.user_email }, (serviceError, dbService) => {
              if (serviceError) {
                return res.status(constants.HTTP_STATUS_INTERNAL_SERVER_ERROR).send({ error: 'Some error occured' });
              }
              if (dbService) {
...
async function tryUploadImage(req: any, res: any) {
  try {
    return await uploadImage('services', req, res);
  } catch (err) {
    return res.status(constants.HTTP_STATUS_INTERNAL_SERVER_ERROR).send({
