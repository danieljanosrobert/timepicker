 ...
 */
export const getBookTime = async (req: any, res: Response) => {
  const serviceId = Base64.decode(req.params.service_id);
  BookTime.findOne({ service_id: serviceId }, (err, dbBookTime) => {
    if (err) {
 ...
 */
export const postSaveBookTime = async (req: any, res: Response) => {
  AdminUser.findOne({ email: req.body.user_email }, (adminUserError, dbUser) => {
    ...
    bcrypt.compare(req.body.password, dbUser.password)
      .then(async (isMatch) => {
        if (isMatch) {
          Service.findOne({ user_email: req.body.user_email }, 'service_id', (serviceError, dbService) => {
            if (serviceError) {
              return res.status(constants.HTTP_STATUS_INTERNAL_SERVER_ERROR).send({ error: 'Some error occured' });
            }
            if (!dbService) {
 ...
 */
export const getBreaks = async (req: any, res: Response) => {
  const serviceId = Base64.decode(req.params.service_id);
  Break.findOne({ service_id: serviceId }, (err, dbBreak) => {
    if (err) {
      return res.status(constants.HTTP_STATUS_INTERNAL_SERVER_ERROR).send({ error: 'Some error occured' });
    }
    if (!dbBreak) {
 ...
 */
export const postSaveBreaks = async (req: any, res: Response) => {
  AdminUser.findOne({ email: req.body.user_email }, (adminUserError, dbUser) => {
    if (adminUserError) {
      return res.status(constants.HTTP_STATUS_INTERNAL_SERVER_ERROR).send({ error: 'Some error occured' });
    }
...
export const getLeaves = async (req: any, res: Response) => {
  const serviceId = Base64.decode(req.params.service_id);
  Leave.findOne({ service_id: serviceId }, (err, dbLeave) => {
    if (err) {
      return res.status(constants.HTTP_STATUS_INTERNAL_SERVER_ERROR).send({ error: 'Some error occured' });
    }
    if (!dbLeave) {
 ...
 */
export const postSaveLeaves = async (req: any, res: Response) => {
  AdminUser.findOne({ email: req.body.user_email }, (adminUserError, dbUser) => {
    if (adminUserError) {
      return res.status(constants.HTTP_STATUS_INTERNAL_SERVER_ERROR).send({ error: 'Some error occured' });
    }
