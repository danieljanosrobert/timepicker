...
export const postGetServiceSettings = async (req: Request, res: Response, next: NextFunction) => {
  Service.findOne({user_email: req.body.user_email})
      .then((dbService) => {
        if (!dbService) {
          return res.status(constants.HTTP_STATUS_NOT_FOUND).send({
...
export const saveService = async (req: any, res: Response, next: NextFunction) => {
